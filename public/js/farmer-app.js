            ...
            html += `
                <div class="panel p-card">
                    ...
                    <div style="display:flex; gap:10px;">
                        <button class="btn btn-sm btn-ghost" onclick="editProduct('${p.id}', '${p.name.replace(/'/g, "\\'")}', '${p.category}', '${p.unit}', ${p.price}, ${p.cost_price}, ${p.stock})">Edit</button>
...
function editProduct(id, name, category, unit, price, cost_price, stock) {
    document.getElementById('product-modal').style.display = 'flex';
...
async function saveProduct() {
    ...
    const data = {
        ...
        cost_price: parseFloat(document.getElementById('f-cost').value),
        stock: parseInt(document.getElementById('f-stock').value),
        emoji: emojiFor(document.getElementById('f-name').value)
    };
        ...
        contentEl.innerHTML = `
            <div class="panel" style="max-width:600px;">
                <div style="display:flex; gap:20px; align-items:center; margin-bottom:20px;">
                    ...
                    <div>
                        <h3>${profile.full_name || 'Farmer'}</h3>
...
async function saveProfile() {
    ...
    try {
        ...
        if(res.ok) {
            toast('Profile saved');
            document.getElementById('farmer-name-pill').textContent = data.full_name || 'Farmer';
        } else {
